...
def import_item_stats(filepath='LOL_items_stats.csv'):
    ...
    try:
        ...
        return {
            'success': True,
            'updated': updated,
            'message': f'更新了 {updated} 件装备的属性数据'
        }
...
def process_match_file(file_content, filename):
    ...
    try:
        ...
        def get_rank_category(rank_fk):
            ...
            return rank_map.get(rank_fk, f'未知({rank_fk})')
...
def get_match_statistics():
    ...
    try:
        ...
        cursor.execute("""
            SELECT
                CASE
                    WHEN game_duration_minutes < 15 THEN '0-15分钟'
                    WHEN game_duration_minutes < 25 THEN '15-25分钟'
                    WHEN game_duration_minutes < 35 THEN '25-35分钟'
                    ELSE '35+分钟'
...
def api_import_items():
    ...
    try:
        ...
        if not os.path.exists('ItemTbl.csv'):
            return jsonify({'success': False, 'error': 'ItemTbl.csv 文件不存在'})
        ...
        if os.path.exists('LOL_items_stats.csv'):
            ...
            result2 = {'success': True, 'updated': 0, 'message': '跳过详细属性导入'}
...
def api_match_upload():
    ...
    if not file.filename.endswith('.csv'):
        return jsonify({'success': False, 'error': '只支持CSV格式文件'})
    ...
    try:
        ...
        try:
            ...
            return jsonify({'success': False, 'error': '文件编码不支持，请使用UTF-8编码'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def api_match_stats():
    ...
    return jsonify({'success': False, 'error': '没有比赛数据'})
...
def api_match_export():
    ...
    try:
        ...
        writer.writerow(['MatchId', 'Patch', 'QueueType', 'RankFk',
                         'GameDuration(秒)', 'GameDuration(分钟)',
